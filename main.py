...
class YandexMarket:
    ...
    def __init__(self, search_text="", dataset_name="", max_pages=5):
        self.init_driver()
    ...
    def get_element_web_driver_wait(self, selector, by=By.CSS_SELECTOR, delay=10):
        # Функция, которая проверяет наличие элемента на странице
    ...
    def parce_page(self, dataset_name):
        name_xpath = lambda \
    ...
    def test(self):
        self.get_main_page()
        self.get_smartphone()
        self.parce_page("Парсинг смартфонов")

    def run(self):
        self.get_main_page()
        self.search(self.search_text)
        self.parce_page(self.dataset_name)
...
def gui_init():
    ...
    def start_button(test=False):
        if test:
            ...
            YandexMarket(search_text=search_text.get(), dataset_name=dataset_name.get(),
                         max_pages=number_page.get()).run()
